from flask import Flask, request, jsonify
import requests
import uuid

app = Flask(__name__)

YOOKASSA_SHOP_ID = "1346746"
YOOKASSA_SECRET_KEY = "live_zkv0c-zUwrtk36xvMD6ylyVcKQh06ST5uh9-Cql7-Kg"
BOT_TOKEN = "8463325671:AAFk8vh9TUf1oBmj2Fip7pxWWK45p579aE0"
XUI_HOST = "144.31.54.21"
XUI_PORT = 58763
XUI_USERNAME = "4WMi0f7K9s"
XUI_PASSWORD = "12345678"
INBOUND_ID = 2

def create_vpn_client(email, days):
    # Твоя существующая функция create_vpn_client (скопируй из bot.py)
    import requests, uuid, json, random, string
    from datetime import datetime, timedelta
    session = requests.Session()
    session.verify = False
    login_url = f"https://{XUI_HOST}:{XUI_PORT}/mYLfcCSnMkPJREgznL/login"
    session.post(login_url, json={"username": XUI_USERNAME, "password": XUI_PASSWORD})
    client_uuid = str(uuid.uuid4())
    expiry = int((datetime.now() + timedelta(days=days)).timestamp() * 1000)
    sub_id = ''.join(random.choices(string.ascii_lowercase + string.digits, k=16))
    client_data = {
        "id": client_uuid, "flow": "", "email": email,
        "limitIp": 0, "totalGB": 0, "expiryTime": expiry,
        "enable": True, "tgId": "", "subId": sub_id, "reset": 0
    }
    add_url = f"https://{XUI_HOST}:{XUI_PORT}/mYLfcCSnMkPJREgznL/panel/inbound/addClient"
    resp = session.post(add_url, data={"id": INBOUND_ID, "settings": json.dumps({"clients": [client_data]})}, headers={'X-Requested-With': 'XMLHttpRequest'})
    if resp.status_code == 200:
        return f"https://tetrisbot.abrdns.com:2096/sub/{sub_id}"
    return None

@app.route('/create_payment', methods=['POST'])
def create_payment():
    data = request.json
    amount = data.get('amount')
    description = data.get('description')
    user_id = data.get('user_id')
    tariff_id = data.get('tariff_id')
    
    idempotence_key = str(uuid.uuid4())
    payment_data = {
        "amount": {"value": f"{amount:.2f}", "currency": "RUB"},
        "payment_method_data": {"type": "bank_card"},
        "confirmation": {"type": "redirect", "return_url": f"https://t.me/testVPNamirbot?start=payment_{idempotence_key}"},
        "description": description,
        "capture": True,
        "metadata": {"user_id": str(user_id), "tariff_id": str(tariff_id)}
    }
    
    auth = (YOOKASSA_SHOP_ID, YOOKASSA_SECRET_KEY)
    headers = {"Idempotence-Key": idempotence_key, "Content-Type": "application/json"}
    response = requests.post("https://api.yookassa.ru/v3/payments", json=payment_data, headers=headers, auth=auth)
    
    if response.status_code == 200:
        result = response.json()
        return jsonify({"payment_url": result["confirmation"]["confirmation_url"], "payment_id": result["id"]})
    return jsonify({"error": response.text}), 400

@app.route('/webhook', methods=['POST'])
def webhook():
    event = request.json
    if event.get('event') == 'payment.succeeded':
        payment = event['object']
        metadata = payment['metadata']
        user_id = int(metadata['user_id'])
        tariff_id = int(metadata['tariff_id'])
        
        # Получаем тариф из БД (можно передать через metadata)
        days = 30 if tariff_id == 2 else 90  # пример, лучше передавать дни в metadata
        email = f"paid_{user_id}_{int(datetime.now().timestamp())}"
        vless_link = create_vpn_client(email, days)
        
        if vless_link:
            # Отправляем сообщение пользователю через бота
            requests.post(
                f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",
                json={"chat_id": user_id, "text": f"✅ Оплата подтверждена!\n🔗 <code>{vless_link}</code>", "parse_mode": "HTML"}
            )
    return "OK", 200

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000)
